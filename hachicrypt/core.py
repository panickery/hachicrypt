...
def split_bitstring_by_prefix(s, split_num):
    i = 0
    result = []
    while i + split_num <= len(s):
        length_bits = s[i:i+split_num]
        i += split_num
        length = int(length_bits, _BASE)  # 2진수 -> 10진수
        if i + length > len(s):
            # 길이 초과시 데이터 부족, break
            break
...
def main() :
    ...
    dec_numbers = [int(ord(ch)) for ch in plain_text]
    print(dec_numbers)
    baseX_chars = [dec_to_baseX(dn) for dn in dec_numbers]
    print(baseX_chars)
    baseX_chars_with_indicator = [put_indicator(ch, _BASE) for ch in baseX_chars]
    print(baseX_chars_with_indicator)
    concat_nums = ''.join(baseX_chars_with_indicator)
    print(concat_nums)
    encoded_str_list = nums_to_char(concat_nums, _INGREDIENTS)
    print(encoded_str_list)
    ...
    print(encoded_str_list)
    concat_nums = chars_to_num(encoded_str_list, _INGREDIENTS)
    print(concat_nums)
    concat_nums = ''.join([str(i) for i in concat_nums])
    print(concat_nums)
    split_prefix_num = split_bitstring_by_prefix(concat_nums, _SIZE_INDICATOR.get(str(_BASE)))
    print(split_prefix_num)
    nums_list = [i[1] for i in split_prefix_num]
    print(nums_list)
    nums_list = [int(i, _BASE) for i in nums_list]
    print(nums_list)
    print(''.join([chr(i) for i in nums_list]))
